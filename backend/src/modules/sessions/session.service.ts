...
async function createSession(
  ...
  if (sessionsCount >= 20000) {
    throw new Error(
      "Session limit reached. Please contact support to increase your limit.",
    );
  }
...
async function endSession(sessionId: string, userId: string) {
  ...
  try {
    ...
    logger.error("Failed to enqueue analysis job (API end)", {
      sessionId,
      userId,
      error: queueError,
    });
...
async function getLiveToken(
  ...
  const resolvedScenarioType = (scenarioType ??
    (session.scenarioType as ScenarioType)) as
    | "technical"
    | "background"
    | "culture";
  ...
  const persona =
    session.persona ??
    (await personaService.getOrCreatePersona(job, resolvedScenarioType));
  ...
  if (!session.personaId && persona?.id) {
    try {
      ...
      logger.warn("Failed to backfill personaId on session", {
        sessionId,
        userId,
        error: e,
      });
  ...
  const systemPrompt = buildLiveInterviewPrompt(
    job,
    persona,
        ...
        config: {
          ...
          speechConfig: {
            voiceConfig: {
              prebuiltVoiceConfig: {
                voiceName:
                  resolvedScenarioType === "background" ? "Aoede" : "Charon",
              },
            },
          },
        },
