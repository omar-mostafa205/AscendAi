...
export const createSession = async (
  ...
  try {
    ...
    Sentry.captureException(error, {
      extra: { userId: req.user.id, jobId: req.params.jobId },
    });
...
export const getSession = async (
  ...
  try {
    ...
    Sentry.captureException(error, {
      extra: { userId: req.user.id, sessionId },
    });
    res.status(500).json({ error: "Internal Server Error" });
  }
};
...
export const getLiveToken = async (
  ...
  try {
    ...
    logger.error("Failed to get live token", {
      error,
      userId: req.user.id,
      sessionId,
    });
    if (
      error instanceof Error &&
      (error.message.startsWith("Session not found") ||
        error.message === "Session job not found")
    ) {
      res.status(404).json({ error: "Session not found" });
      return;
    }
