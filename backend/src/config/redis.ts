...
redisQueue.on("reconnecting", (time: number) => logger.warn("Redis reconnecting", { time }))
redisQueue.on("end", () => logger.warn("Redis connection ended"))
redisQueue.on("error", (err) => logger.error("Redis error", { error: err?.message ?? String(err) }))
