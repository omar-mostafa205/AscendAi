...
export const registerSessionHandlers = (io: Server, socket: Socket) => {
  ...
  const flushPendingMessages = async (sessionId: string) => {
    ...
    try {
      ...
      if (!session) {
        logger.warn("flushPendingMessages: session not found", {
          sessionId,
          userId,
        });
        return;
  ...
  const enqueueMessage = (sessionId: string, message: SessionMessage) => {
    ...
    const timer = setTimeout(() => {
      flushPendingMessages(sessionId).catch((error) => {
        Sentry.captureException(error);
        logger.error("flushPendingMessages error", {
          sessionId,
          userId,
          error,
        });
  ...
  socket.on("join_session", async (payload: JoinPayload) => {
    ...
    try {
      ...
      const existingSocketId =
        activeSocketIdBySessionUserKey.get(sessionUserKey);
  ...
  socket.on("save_message", async (payload: SaveMessagePayload) => {
    ...
    if (!sessionId || !role || !content?.trim()) {
      logger.debug("save_message: ignored empty payload", {
        sessionId,
        userId,
        role,
      });
    ...
    for (const sessionId of joinedSessionIds) {
      ...
      const timer = setTimeout(async () => {
        try {
          ...
          if (roomSize > 0) {
            logger.debug("Session still has active connections", {
              sessionId,
              roomSize,
            });
            return;
          ...
          if (
            ...
            logger.debug("Session not active, skipping auto-end", {
              sessionId,
          ...
          const messages = Array.isArray(refreshed?.messages)
            ? (refreshed?.messages as any[])
            : [];
